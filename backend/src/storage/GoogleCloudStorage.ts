...
export class GoogleCloudStorage extends BaseStorage {
    ...
    async copyFile(params: { bucketName: string; srcKey: string; destinationKey: string }) {
    ...
    async getFileMetadata(params: { bucketName: string; key: string }) {
        const { key } = params;
        const fullPath = this.getFullPath(key);
    ...
    async createWriteStream(params: { bucketName: string; key: string; contentType?: string }) {
        const { key, contentType } = params;
